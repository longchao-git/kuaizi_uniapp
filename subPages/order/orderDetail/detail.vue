<template>
	<view>
		<view class="page">
			<view class="order-head-nav" :style="'top:' + sysinfo.statusBarHeight + 'px'">
				<view class="order-head-back" @tap="goBack">
					<image src="/static/image/icon_back.png"></image>
				</view>
			</view>
			<view class="order-head" v-if="orderdetail.waimai && orderdetail.waimai.shop_show">
				<image class="order-head-img" mode="aspectFill" :src="orderdetail.waimai.shop_show"></image>
				<view
					:style="'width:750rpx;height:' + sysinfo.statusBarHeight + 'px;position: fixed;top: 0;left: 0;right: 0;z-index: 200;'">
				</view>
				<view class="order-head-mask"></view>
				<view class="order-head-title">{{ orderdetail.waimai.title }}</view>
			...
			<block>
				<view class="orderStatus">
					...
					<view class="ordertip">
						...
						<text v-else-if="orderdetail.order_status == -1">订单已取消,欢迎再次光临</text>
						<text v-else-if="orderdetail.show_btn.pay == 1">订单将在<text style="color: #F8627F;"> {{ mytime }}
							</text>后自动取消请尽快支付</text>
						<text v-else-if="orderdetail.order_status == 2">{{ orderdetail.pei_type == 3 ? '等待自提中' :
							'请耐心等待送达' }}</text>
					</view>
					...
					<view class="status_btn">
						...
						<block v-if="orderdetail.show_btn.confirm == '1'">
							<button type="default" :size="defaultSize" @tap="confirmpop"
								hover-class="other-button-hover">{{ orderdetail.pei_type == 3 ? '确认自提' :
									'确认送达' }}</button>
						</block>
			...
			<view class="orderDetail orderDetailViewNew">
				<view class="orderDetail-title">你的订单</view>
				<view class="pub_list tit mt10" @tap="todetail" :id="orderdetail.waimai.shop_id">
					...
					<view class="right_box">
						...
						<ul>
							<li v-for="(item, index2) in item.product" :key="index2" class="pub_list">
								...
								<view class="price">€{{ item.product_oldprices }}<text class="small">€{{
									item.product_prices }}</text></view>
							</li>
						</ul>
					</view>
				</view>
			...
			<view class="caidanListsOne textborder" v-else>
				<ul v-for="(item, index) in goods_lis" :key="index">
					<li v-for="(item, index2) in item.product" :key="index2" class="pub_list ">
						...
						<view class="pub_list_bd">
							<view class="tit ">
								<view class="beyond2">{{ item.product_name }}</view>
								<view class="spec-box">
						...
						<view class="num">x {{ item.product_number }}</view>
						<view class="price">€{{ item.product_prices }}<text class="small">€{{ item.product_oldprices
						}}</text></view>
					</li>
				</ul>
			</view>
					...
					if (res.error == '0') {
						...
						app.globalData.ordersDetail({
							...
							if (res.error == '0') {
								...
								app.globalData.topage('/subPages/order/orderDetail/detail?orderid=' +
									orderid);
							}
						});
					} else {
...
<style>
	.textborder {
		border-bottom: 1rpx #E6EAED solid;
	}
	...
	.order-head-mask {
		...
		top: 0;
		bottom: 0;
		background: linear-gradient(180deg, rgba(0, 0, 0, 0.55) 0%, rgba(0, 0, 0, 0.2) 45%, rgba(0, 0, 0, 0.6) 100%);
		z-index: 1;
	}
	...
	.order-head-title {
		...
		font-size: 40rpx;
		font-weight: 600;
		text-shadow: 0 4rpx 10rpx rgba(0, 0, 0, 0.35);
	}
