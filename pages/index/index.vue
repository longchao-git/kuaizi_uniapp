		...
		<skeleton :loading="loading" :showAvatar='false' :row="skeleton1.row" :showTitle="skeleton1.showTitle">
			<view>
				...
				<block data-type="template" data-is="showToast" data-attr="showToast: showToast">
					<block v-if="isShowToast(showToast)">
		...
		methods: {
			...
			https: function(nothasloading) {
				...
				app.globalData.homeInfo({}, function(ret) {
					...
					if (ret.error == '0') {
						...
						that.setData({
							allCates: ret.data.waimai_cates
						});
			...
			tapScreen(e) {
				...
				if (e.currentTarget.id != '' && this.screenTabIndex != e.currentTarget.id) {
					this.setData({
						newscroTabIndex: e.currentTarget.id,
						screenTabIndex: e.currentTarget.id,
						screenTabShow: true //头部下拉是否出现

					});
				} else {
					this.setData({
						screenTabIndex: null,
						screenTabShow: false //头部下拉是否出现
			...
			tapCates(e) {
				...
				if (e.currentTarget.dataset.index != 0) {} else {
					this.setData({
						...
						//头部下拉是否出现
						page: 1,
						cateVal: this.allCates[catesIndex].title
					});
			...
			tapCate(e) {
				...
				this.setData({
					...
					//头部下拉是否出现
					page: 1,
					cateVal: this.allCates[this.catesIndex].title
				});
			...
			tapPai(e) {
				this.setData({
					paiIndex: e.currentTarget.dataset.idx,
					screenTabIndex: null,
					screenTabShow: false,
					//头部下拉是否出现
					page: 1,
					paiVal: this.pais[e.currentTarget.dataset.idx].title
				});
